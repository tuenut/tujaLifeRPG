...
//Get date
window.API.subscribe.get_date = (callback) => {
  ipcRenderer.on('get_date-reply', (event, arg) => callback(arg));
};
...
window.API.unsubscribe.get_date = (callback) => {
  ipcRenderer.removeListener('get_date-reply', callback);
};


//Create character
window.API.subscribe.create_character = (callback) => {
  ipcRenderer.on('create_character-reply', (event, arg) => callback(arg));
};
...
window.API.unsubscribe.create_character = (callback) => {
  ipcRenderer.removeListener('create_character-reply', callback);
};
